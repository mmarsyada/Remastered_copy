...
mission_level_choice = ScreenPlay:new {
	...
	levels = {
		...
		{levelRange = "Easy", levelSelect = 1},
		{levelRange = "Med 1", levelSelect = 2}, 
		{levelRange = "Med 2", levelSelect = 12}, 
		{levelRange = "Med 3", levelSelect = 25}, 
...
function  mission_level_choice:levelSelection(pPlayer, pSui, eventIndex, args)
	...
	if (tonumber(selectedLevel) == 0) then
		CreatureObject(pPlayer):sendSystemMessage("You have selected: " .. selectedRange .. ".  You may now choose missions suited to your skill or group level.")
	else	
		CreatureObject(pPlayer):sendSystemMessage("You have selected: " .. selectedRange .. ".  These missions would be considered suitable for a player group level of " .. selectedLevel .. ".")
