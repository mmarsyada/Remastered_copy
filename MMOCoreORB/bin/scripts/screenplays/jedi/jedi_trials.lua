...
function JediTrials:unlockJediPadawan(pPlayer, dontSendSui)
	...
	broadcastGalaxy("\\#00ff00IMPERIAL COMMUNICATIONS CENTER: Lord Vader has detected a disturbance in the Force. Be on the lookout for persons that exhibit odd or very unique abilities. The Governor has authorized the deadly use of force to erradicate this threat from the galaxy. Monetary Rewards will be posted.")
...
function JediTrials:unlockJediKnight(pPlayer)
	...
	if not CustomTefManager:enabled() or CustomTefManager:isPermaOvert(CreatureObject(pPlayer)) then
