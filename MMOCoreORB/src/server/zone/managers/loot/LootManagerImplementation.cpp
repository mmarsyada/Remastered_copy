...
bool LootManagerImplementation::loadConfigData() {
	...
	crystalTable = luaObject.getObjectField("lightsaber_module_named_force_crystal");
	crystal = new CrystalData();
	crystal->readObject(&crystalTable);
	crystalData.put("lightsaber_module_named_force_crystal", crystal);
	crystalTable.pop();

	crystalTable = luaObject.getObjectField("lightsaber_module_unamed_force_crystal");
	crystal = new CrystalData();
	crystal->readObject(&crystalTable);
	crystalData.put("lightsaber_module_unamed_force_crystal", crystal);
	crystalTable.pop();
...
TangibleObject* LootManagerImplementation::createLootObject(const LootItemTemplate* templateObject, int level, bool maxCondition) {
	...
	if (prototype->isLightsaberCrystalObject()) {
		...
		if (crystal != nullptr){
			...
			// Enable rare modifications to crystals
			crystal->setExcMod(excMod);
		}
	}
