	...
	if (band == baseData->getUplinkBand()) {
		...
		if (isBandIdentified(building)) {
			...
			if (building->getPvpStatusBitmask() & CreatureFlag::OVERT){
				...
				if (building->getFaction() == Factions::FACTIONREBEL){
					String str = "A REBEL PVP Base is Under Attack! @ " + zone->getZoneName() + " " + String::valueOf(building->getPositionX()) + " " + String::valueOf( building->getPositionZ()) + " " + String::valueOf( building->getPositionY()) + ".";
					zone->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, str);
				}
				else {
					String str = "An IMPERIAL PVP Base is Under Attack! @ " + zone->getZoneName() + " " + String::valueOf(building->getPositionX()) + " " + String::valueOf( building->getPositionZ()) + " " + String::valueOf( building->getPositionY()) + ".";
					zone->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, str);
				}
				...
				if (building->getFaction() == Factions::FACTIONREBEL){
					String str = "A REBEL PVE Base is Under Attack! @ " + zone->getZoneName(); // + " " + String::valueOf(building->getPositionX()) + " " + String::valueOf( building->getPositionZ()) + " " + String::valueOf( building->getPositionY()) + ".";
					zone->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, str);
				}
				else {
					String str = "An IMPERIAL PVE Base is Under Attack! @ " + zone->getZoneName(); // + " " + String::valueOf(building->getPositionX()) + " " + String::valueOf( building->getPositionZ()) + " " + String::valueOf( building->getPositionY()) + ".";
					zone->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, str);
				}
			}
			return;
...
void GCWManagerImplementation::doBaseDestruction(BuildingObject* building) {
	...
	if (baseType == STATICFACTIONBASE) {
		...
		if (owner != nullptr && owner->isPlayerCreature()) {
			...
			if (building->getFaction() == Factions::FACTIONREBEL){
				building->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, "\\#FF9933 ATTENTION REBELS, YOUR BASE HAS BEEN DESTROYED!!");
			} else if (building->getFaction() == Factions::FACTIONIMPERIAL){
				building->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, "\\#7133FF ATTENTION IMPERIALS, YOUR BASE HAS BEEN DESTROYED!!");
			}
		}
	}
