...
void CreatureManagerImplementation::droidHarvest(Creature* creature, CreatureObject* droid, int selectedID, int harvestBonus) {
	...
	if (owner->hasSkill("gcw_ranger_novice")){
		profBonus += 5; // + 5% harvest bonus
		if (owner->hasSkill("gcw_ranger_05")){
			quantityExtracted++; // increase creature resource by 1
			if (owner->hasSkill("gcw_ranger_08")){
				profBonus += 5; // + 10% harvest bonus
				if (owner->hasSkill("gcw_ranger_master")){
					quantityExtracted++; // increase creature resource by 1
				}
			}
		}
