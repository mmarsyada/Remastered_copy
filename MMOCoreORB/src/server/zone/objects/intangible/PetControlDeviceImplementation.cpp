...
void PetControlDeviceImplementation::callObject(CreatureObject* player) {
	...
	ManagedReference<PlayerObject*> ghost = player->getPlayerObject();
        ...
        if (pet->isMount() && player->isInCombat()){
	    player->sendSystemMessage("You cannot call a mount while in combat");
            return;
	}
	...
	for (int i = 0; i < ghost->getActivePetsSize(); ++i) {
		...
		if (object != nullptr) {
			if (object->isCreature() && petType == PetManager::CREATUREPET) {
				...
				if (++currentlySpawned >= maxPets || 
                                    (activePetTemplate->getTemplateName() == "at_st" && callingPetTemplate->getTemplateName() == "at_st")  ||
